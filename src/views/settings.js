		...
		<div className={classes.root}>
			...
			<div className={classes.input}>
				...
				<InputText
					...
					wide
					onChange={value =>
						dispatch({ type: 'setDuration', value: Number(value) })
					}
				/>
			...
			<div className={classes.input}>
				...
				<Toggle
					...
					onChange={() =>
						dispatch({
							type: 'setUseDefaultStretches',
							value: !useDefaultStretches,
						})
					}
				/>
			</div>
			{!_.isEmpty(stretchesMap) &&
				_.map(_.concat(stretchesMap, newStretch), (stretch, i) => {
					const isFirstStretch = i === 0;
					const isLastExistingStretch =
						i === _.size(stretchesMap) - 1;
					const isNewStretch = i === _.size(stretchesMap);

					return (
						<div
							className={clsx(
								classes.stretchMapInput,
...
const useInputTextStyles = makeStyles({
	...
	input: {
		...
		borderWidth: '1px',
		padding: '2px',
		width: ({ isTypeNumber, wide }) =>
			isTypeNumber ? (wide ? '4rem' : '2rem') : wide ? '12rem' : '7rem',
		'& :active, :hover, :focus': {
	...
	return (
		<div>
			...
			<ul className={classes.list}>
				<li>
					<b>Name:</b> The name which will show up for you in the
					queue.
				</li>
				<li>
					<b>Frequency:</b> The number of times the stretch will
					repeat in the queue. If frequencies above 1 are used, then
					the higher frequency stretches will be added to the end in
					the same order as originally until the frequency has been
					hit.
				</li>
				<li>
					<b>Both Sides:</b> A single entry or a single entry for each
					side of your body. Text will either: just be the name, or
					for both sided stretches, it will be &apos;Stretch
					Left/Right *name*&apos;.
				</li>
			</ul>
		</div>
